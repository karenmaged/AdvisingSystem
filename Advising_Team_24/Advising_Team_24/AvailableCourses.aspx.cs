...
namespace Advising_Team_24
{
    public partial class AvailableCourses : System.Web.UI.Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            if (string.Equals(sem, "Select Semester", StringComparison.OrdinalIgnoreCase))
            {
                Response.Write("please choose a semester");
            }
            else
            {
                SqlCommand opt = new SqlCommand("SELECT * FROM FN_SemsterAvailableCourses(@semstercode)", conn);
                opt.CommandType = CommandType.Text;
                opt.Parameters.Add(new SqlParameter("@semstercode", sem));
                ...
                try
                {
                    ...
                    {
                        Label thecoursename = new Label();
                        thecoursename.Text = "No Available courses in this semester";
                        exist.Controls.Add(thecoursename);
                    }
                }
                catch (Exception ex)
