...
{
    public partial class ApproveRejectCourse : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                SqlCommand pop = new SqlCommand("select request_id from request where type='course'", conn);
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand st = new SqlCommand("Procedures_AdvisorApproveRejectCourseRequest", conn);
            st.CommandType = CommandType.StoredProcedure;
            st.Parameters.Add(new SqlParameter("@requestID", temp));
            st.Parameters.Add(new SqlParameter("@current_semester_code", temp1));
            conn.Open();
            if (temp1 == "Select Semester" || temp == "Select Request")
            {
